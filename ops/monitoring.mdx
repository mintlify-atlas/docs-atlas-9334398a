...
| `missedTicks` | Ticks skipped because the previous tick ran too long |
...
<Tip>
  Watch `gameSimulationOverBudgetTicks` and `missedTicks` as your primary at-a-glance health signals. A single non-zero `missedTicks` reading means the simulation loop fell a full 33 ms behind — investigate `gameSimulationTickMaxMicros` and per-phase timings first before tuning thread counts.
</Tip>
